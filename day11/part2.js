...
const round = (monkeys, i) => {
  let mod = R.reduce(R.multiply, 1, R.map(x => x.test, monkeys));
  for(let monkey of monkeys) {
    for(let item of monkey.items) {
      let operand = monkey.operand === 'old' ? item : monkey.operand;
      let newItem = operations[monkey.operator](item, operand) % mod;
...
export default R.pipe(parseInput, R.reduce(round, R.__, R.range(1, 10001)), getScore);